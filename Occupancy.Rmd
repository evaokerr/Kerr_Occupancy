...
# Scale using only observed values (global z-score across all observed entries)
ratio_vec <- as.numeric(ratio_mat)
ratio_mean <- mean(ratio_vec, na.rm = TRUE)
ratio_sd   <- sd(ratio_vec,   na.rm = TRUE)
if (is.na(ratio_sd) || ratio_sd == 0) ratio_sd <- 1  # safety for tiny samples

ratio_sc <- (ratio_mat - ratio_mean) / ratio_sd

# Replace NAs with 0 AFTER scaling so missing cells contribute 0 to the linear predictor
ratio_sc0 <- ratio_sc
ratio_sc0[is.na(ratio_sc0)] <- 0
...
params <- c(
  ...
  "beta_yr_dip","beta_yr_call","occ_prop","psi","z"
)
