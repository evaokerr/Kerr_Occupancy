...
# Force a common projected CRS in meters (CONUS Albers)
target_crs <- 5070

# Reproject BOTH centroid layers to the same CRS
ceylon_cent_c <- st_transform(ceylon_cent, target_crs)
jones_cent_c  <- st_transform(jones_cent,  target_crs)

# 1) Attach residuals to centroids
ceylon_cent_c$residual <- res_ceylon
jones_cent_c$residual  <- res_jones

# 2) Combine for a single faceted plot (keeps geometries)
ceylon_cent_c$SiteName <- "Ceylon WMA"
jones_cent_c$SiteName  <- "Jones Center Ichauway"

both_cent <- rbind(
  ceylon_cent_c[, c("residual", "SiteName", "geometry")],
  jones_cent_c[,  c("residual", "SiteName", "geometry")]
)

# 3) Standardize residuals for a common color scale (z-scores)
both_cent$res_z <- as.numeric(scale(both_cent$residual))
